            ...
            <div class='flotar'>
                <div style='float: right;'>
                  ...
                  <v-btn                                        
                    color="primary"
                    class="mr-4" absolute
                    @click="report_action_btn">Generar reporte</v-btn>
                </div>                          
            </div>
            ...
            <v-data-table :headers="headers" :items="items" :search="search">
               <template v-slot:item="row">
                <tr>
                  ...
                  <td>{{row.item.sede}}</td>
                  <td>{{row.item.departamento}}</td>               
                  <td>{{row.item.status}}</td>                                    
                  <td>{{row.item.updated_at}}</td> 
                  <td>{{row.item.days_difference}}</td>                                                      
                  <td>
                    <div v-if="row.item.status != 'EN PROCESO'">   
                    <v-btn small color="transparent" @click="bitacora(row.item.registro)"><i class="fa fa-file-pdf" style="color:rgb(203, 43, 43)"></i></v-btn>                                                                                                                                                                            
                    </div>
                  </td>                
                </tr>
            </template>
      ...
      <div class="modal-mask">
        ...
        <div class="modal-dialog">
          ...
          <br>
              ...
              <div class="modal-body">
                       <button type="button" class="close" @click="pdfModal=false"><span aria-hidden="true">&times;</span></button>
                  <div class="form-group">                    
                  <iframe :src="archivoPDF" style="width:90%; height:400px;" frameborder="0"></iframe>
                  </div>
              </div>                                           
...
export default {
    ...
    data () {
      ...
      field: {
        ...
        headers: [          
          ...
          { text: 'Cantidad Dias', value: 'days_difference' },
    ...
    created: function() {
     ...
     methods: {  
       ...
       report_action_btn: function(e){  
        console.log("Generando reporte con " + this.report_action);
        axios.post('/listado_reporte_ticket', {action:this.report_action})
          .then((response) =>{                                    
       ...
       archivos: function(expediente) {                              
        ...
        bitacora:function(id)   {   
          ...
          this.mModel = true;   */
          console.log("El registro a consultar es: " + id);
          axios.post('/ticket/generarBoleta', {registro:id})
          .then((response) =>{                                    
            //this.items = response.data;     
            //console.log("Respuesta exp: " +  JSON.stringify(response.data,null,2));       
            //console.log("Respuesta exp: " +  response.data);       
            //window.location.href = response.data;            
            this.pdfModal = true;
            this.archivoPDF ='/pdf/'+response.data;    
          });
